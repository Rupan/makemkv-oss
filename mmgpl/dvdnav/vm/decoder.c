...
static uint16_t get_GPRM(registers_t* registers, uint8_t reg) {
  if (0!=(registers->GPRM_mode&(1<<reg))) {
    uint16_t result;
    /* Counter mode */
    /* fprintf(MSG_OUT, "libdvdnav: Getting counter %d\n",reg);*/
    result = (uint16_t)(((registers->time_counter - registers->GPRM_time[reg])*32) / 5625);
...
static uint16_t eval_reg(command_t* command, uint8_t reg) {
  if(reg & 0x80) {
    ...
    command->registers->SPRM_flags |= (1 << reg);
    if (reg >= 24) return 0;
    return command->registers->SPRM[reg];
  } else {
...
//#undef MSG_OUT
//#undef fprintf
#define tfprintf vm_debug_print
#define tMSG_OUT log
...
void vm_print_link(const trace_log_t* log, const link_t* value) {
  ...
  case LinkTailPGC:
  case LinkRSM:
    tfprintf(tMSG_OUT, "libdvdnav: %s (button %d)\n", cmd, value->data1);
    break;
  case LinkPGCN:
  case JumpTT:
  case JumpVTS_TT:
  ...
  case JumpSS_VMGM_PGC:
    tfprintf(tMSG_OUT, "libdvdnav: %s %d\n", cmd, value->data1);
    break;
  case LinkPTTN:
  case LinkPGN:
  case LinkCN:
    tfprintf(tMSG_OUT, "libdvdnav: %s %d (button %d)\n", cmd, value->data1, value->data2);
    break;
  case Exit:
  case JumpSS_FP:
  case PlayThis: /*  Humm.. should we have this at all.. */
    tfprintf(tMSG_OUT, "libdvdnav: %s\n", cmd);
    break;
  case JumpVTS_PTT:
    tfprintf(tMSG_OUT, "libdvdnav: %s %d:%d\n", cmd, value->data1, value->data2);
    break;
  case JumpSS_VTSM:
    tfprintf(tMSG_OUT, "libdvdnav: %s vts %d title %d menu %d\n",
            cmd, value->data1, value->data2, value->data3);
    break;
  case CallSS_FP:
    tfprintf(tMSG_OUT, "libdvdnav: %s resume cell %d\n", cmd, value->data1);
    break;
  case CallSS_VMGM_MENU: /*  == 2 -> Title Menu */
  case CallSS_VTSM:
    tfprintf(tMSG_OUT, "libdvdnav: %s %d resume cell %d\n", cmd, value->data1, value->data2);
    break;
  case CallSS_VMGM_PGC:
    tfprintf(tMSG_OUT, "libdvdnav: %s %d resume cell %d\n", cmd, value->data1, value->data2);
    break;
...
void vm_print_registers(const trace_log_t* log, registers_t *registers ) {
  int32_t i;
  tfprintf(tMSG_OUT, "libdvdnav:    #   ");
  for(i = 0; i < 24; i++)
    tfprintf(tMSG_OUT, " %2d |", i);
  tfprintf(tMSG_OUT, "\nlibdvdnav: SRPMS: ");
  for(i = 0; i < 24; i++)
    tfprintf(tMSG_OUT, "%04x|", registers->SPRM[i]);
  tfprintf(tMSG_OUT, "\nlibdvdnav: GRPMS: ");
  for(i = 0; i < 16; i++)
    tfprintf(tMSG_OUT, "%04x|", get_GPRM(registers, i) );
  tfprintf(tMSG_OUT, "\nlibdvdnav: Gmode: ");
  for(i = 0; i < 16; i++)
    tfprintf(tMSG_OUT, "%04x|", (0!=(registers->GPRM_mode&(1<<i)))?1:0 );
  tfprintf(tMSG_OUT, "\nlibdvdnav: Gtime: ");
  for(i = 0; i < 16; i++)
    tfprintf(tMSG_OUT, "%04lx|", ((registers->GPRM_time[i]<<5)/5625) & 0xffff);
  tfprintf(tMSG_OUT, "\n");
}
  ...
  switch(vm_getbits(command, 59, 4)) {
    ...
    case 3: /*  Mode: Counter / Register + Set */
      ...
      if(vm_getbits(command, 23, 1)) {
        command->registers->GPRM_mode |= (1 << data2); /* Set bit n */
      } else {
        command->registers->GPRM_mode &= ~(1 << data2);/* Reset bit n */
      }
      if(cond) {
...
int32_t vmEval_CMD(const trace_log_t* log, vm_cmd_t commands[], int32_t num_commands,
               registers_t *registers, link_t *return_values) {
  int32_t i = 0;
  int32_t total = 0;

#ifdef TRACE
  if ((NULL != log) && (NULL != log->logcb.pf_log)) {
      /*  DEBUG */
      tfprintf(tMSG_OUT, "libdvdnav: Registers before transaction\n");
      vm_print_registers(log, registers);
      tfprintf(tMSG_OUT, "libdvdnav: Full list of commands to execute\n");
      for (i = 0; i < num_commands; i++)
          vm_print_cmd(log, i, &commands[i]);
      tfprintf(tMSG_OUT, "libdvdnav: --------------------------------------------\n");
      tfprintf(tMSG_OUT, "libdvdnav: Single stepping commands\n");
  }
#endif
...
#ifdef TRACE
        if ((NULL != log) && (NULL != log->logcb.pf_log)) {
            tfprintf(tMSG_OUT, "libdvdnav: Registers after transaction\n");
            vm_print_registers(log, registers);
            tfprintf(tMSG_OUT, "libdvdnav: eval: Doing Link/Jump/Call\n");
        }
#endif
      return 1;
    }
...
static char g_log_line[MaxLogLineSize+1];
static unsigned int g_log_size = 0;

static void vm_debug_println(const trace_log_t* log, const char* str)
{
    va_list lst;

    memset(&lst,0,sizeof(lst));

    (*log->logcb.pf_log)(log->priv, DVDNAV_LOGGER_LEVEL_INFO, g_log_line, lst);
}

void vm_debug_print(const trace_log_t* log, const char* fmt, ...)
{
    unsigned int log_have,sz;
    va_list lst;

    if (NULL == log) return;
    if (NULL == log->logcb.pf_log) return;

    log_have = MaxLogLineSize - g_log_size;

    if (log_have < 2)
    {
        g_log_line[g_log_size] = 0;
        vm_debug_println(log,g_log_line);
        g_log_size = 0;
        log_have = MaxLogLineSize;
    }

    va_start(lst, fmt);
    sz = vsnprintf(g_log_line + g_log_size, log_have, fmt, lst);
    va_end(lst);
    ...
    while (g_log_size!=0)
    {
        ...
        *p = 0;
        ...
        log_have = (unsigned int)((p + 1) - g_log_line);

        if (log_have <= g_log_size)
        {
            g_log_size -= log_have;
            memmove(g_log_line, g_log_line + log_have, g_log_size);
        } else {
            g_log_size = 0;
        }
        g_log_line[g_log_size] = 0;
