...
int vm_reset(vm_t *vm, const char *dvdroot,
  ...
  vm->state.rnd = 0;
  rand_seed(&vm->state.rnd, vm->vmgi->vmgi_mat->vmg_last_sector);
  rand_seed(&vm->state.rnd, vm->vmgi->vmgi_mat->vmgi_last_sector);
  rand_seed(&vm->state.rnd, vm->vmgi->vmgi_mat->vmgm_vobu_admap);
...
int vm_exec_cmd(vm_t *vm, vm_cmd_t *cmd) {
  ...
  if(vmEval_CMD((trace_log_t*)vm, cmd, 1, &vm->state.registers, &link_values))
    return process_command(vm, link_values);
  else
...
static int process_command(vm_t *vm, link_t link_values) {
  ...
  while(link_values.command != PlayThis) {
...
#ifdef TRACE
    if ((NULL!=vm->priv) && (NULL!=vm->logcb.pf_log)) {
      Log3(vm, "Before printout starts:");
      vm_print_link((const trace_log_t*)vm, &link_values);
    ...
    case LinkPrevPG:
      ...
      /* BUTTON number:data1 */
      if ((1 == vm->state.pgN) || (0 != vm->state.pgc->pg_playback_mode)) {
    ...
    case LinkNextPGC:
      ...
      if(link_values.data1 != 0)
        vm->state.HL_BTNN_REG = link_values.data1 << 10;
      vassert(vm->state.pgc->next_pgc_nr != 0);
    ...
    do_LinkPrevPGC:
      ...
      if(link_values.data1 != 0)
        vm->state.HL_BTNN_REG = link_values.data1 << 10;
      vassert(vm->state.pgc->prev_pgc_nr != 0);
    ...
    case LinkGoUpPGC:
      ...
      if(link_values.data1 != 0)
        vm->state.HL_BTNN_REG = link_values.data1 << 10;
      vassert(vm->state.pgc->goup_pgc_nr != 0);
    ...
    case LinkRSM:
        ...
        if(vm->state.rsm_cellN == 0) {
          vassert(vm->state.cellN); /*  Checking if this ever happens */
          vm->state.pgN = 1;
          link_values = play_PG(vm);
        } else {
    ...
    case LinkPTTN:
      ...
      vassert(vm->state.domain == DVD_DOMAIN_VTSTitle);
    ...
    case JumpTT:
      ...
      /* Stop SPRM9 Timer */
      /* Set SPRM1 and SPRM2 */
      vassert(vm->state.domain == DVD_DOMAIN_VMGM || vm->state.domain == DVD_DOMAIN_FirstPlay); /* ?? */
    ...
    case JumpVTS_TT:
      ...
      /* Stop SPRM9 Timer */
      /* Set SPRM1 and SPRM2 */
      vassert(vm->state.domain == DVD_DOMAIN_VTSMenu || vm->state.domain == DVD_DOMAIN_VTSTitle); /* ?? */
    ...
    case JumpSS_FP:
      ...
      vassert(vm->state.domain == DVD_DOMAIN_VMGM || vm->state.domain == DVD_DOMAIN_VTSMenu); /* ?? */
      if (!set_FP_PGC(vm))
        vassert(0);
      link_values = play_PGC(vm);
      break;
    ...
    case JumpSS_VTSM:
      ...
      /* get_MENU:data3 */
      if(link_values.data1 != 0) {
          vassert(vm->state.domain == DVD_DOMAIN_VTSMenu ||
