...
link_t play_PGC(vm_t *vm) {
  ...
  if ((vm->state).pgc->pg_playback_mode!=0) {
    rand_next(&vm->state.rnd);
    (vm->state).pgN_step = 0;
    (vm->state).pgN = 1 + rand_shuffle(&vm->state.rnd, (vm->state).pgc->nr_of_programs, (vm->state).pgN_step);
  }
  ...
  if((vm->state).pgc->command_tbl && (vm->state).pgc->command_tbl->nr_of_pre) {
    if(vmEval_CMD((trace_log_t*)vm, (vm->state).pgc->command_tbl->pre_cmds,
...
link_t play_PGC_post(vm_t *vm) {
  ...
  if((vm->state).pgc->command_tbl && (vm->state).pgc->command_tbl->nr_of_post &&
     vmEval_CMD((trace_log_t*)vm, (vm->state).pgc->command_tbl->post_cmds,
...
#endif
    vassert((vm->state).pgN == (vm->state).pgc->nr_of_programs + 1);
...
link_t play_Cell(vm_t *vm) {
  ...
  case 0: /*  Normal */
    vassert((vm->state).pgc->cell_playback[(vm->state).cellN - 1].block_type == 0);
    break;
...
#ifdef DVDNAV_STRICT
      vassert((vm->state).cellN <= (vm->state).pgc->nr_of_cells);
      vassert((vm->state).pgc->cell_playback[(vm->state).cellN - 1].block_mode != 0);
      vassert((vm->state).pgc->cell_playback[(vm->state).cellN - 1].block_type == 1);
#else
      if (!((vm->state).cellN <= (vm->state).pgc->nr_of_cells) ||
...
#endif
      if(vmEval_CMD((trace_log_t*)vm, &(vm->state).pgc->command_tbl->cell_cmds[cell->cell_cmd_nr - 1], 1,
                    &(vm->state).registers, &link_values)) {
        if (link_values.command != LinkNoLink) {
          return link_values;
        } else {
#ifdef TRACE
            Log3(vm, "Cell command did a Cheesy Link");
#endif
        }
      } else {
#ifdef TRACE
...
link_t play_PGN(vm_t *vm) {
  int new_pgN;
  int last_pg;

  vassert((vm->state).pgc != NULL);

  last_pg = 0;
  new_pgN = get_PGN(vm);

  if (0 != (vm->state).pgc->pg_playback_mode) {
      if (new_pgN != (vm->state).pgN) {
        unsigned int pgCnt = ((vm->state).pgc->pg_playback_mode & 0x7f) + 1;

        (vm->state).pgN_step++;
        if ((vm->state).pgN_step >= pgCnt) {
            last_pg = 1;
        } else {
          new_pgN = 1 + rand_shuffle(&vm->state.rnd, (vm->state).pgc->nr_of_programs, (vm->state).pgN_step);
          (vm->state).cellN = (vm->state).pgc->program_map[new_pgN - 1];
        }
      }
  } else {
