...
typedef struct {
  uint16_t SPRM[24];
  uint16_t GPRM[16];
  uint16_t GPRM_mode;  /* Need to have some thing to indicate normal/counter mode for every GPRM */
  uint32_t GPRM_time[16];  /* For counter mode */
  uint32_t SPRM_flags;
...
/* identical layout to vm_t. GPL is cancer. */
typedef struct {
    void *priv;
