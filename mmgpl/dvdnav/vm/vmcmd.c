...
#undef MSG_OUT
#undef fprintf
#define fprintf vm_debug_print
#define MSG_OUT log

static void print_system_reg(const trace_log_t* log, uint16_t reg) {
...
static void print_g_reg(const trace_log_t* log, uint8_t reg) {
    if(reg < 16)
...
static void print_reg(const trace_log_t* log, uint8_t reg) {
  if(reg & 0x80)
    print_system_reg(log, reg & 0x7f);
  else
    print_g_reg(log, reg & 0x7f);
}

static void print_cmp_op(const trace_log_t* log, uint8_t op) {
...
static void print_set_op(const trace_log_t* log, uint8_t op) {
...
static void print_reg_or_data(const trace_log_t* log, command_t* command, int immediate, int start) {
  if(immediate) {
    ...
    print_reg(log, vm_getbits(command, start - 8, 8));
  }
}

static void print_reg_or_data_2(const trace_log_t* log, command_t* command, int immediate, int start) {
  if(immediate)
...
static void print_reg_or_data_3(const trace_log_t* log, command_t* command, int immediate, int start) {
  if(immediate) {
    uint32_t i = vm_getbits(command, start, 16);
    ...
    print_reg(log, vm_getbits(command, start, 8));
  }
}


static void print_if_version_1(const trace_log_t* log, command_t* command) {
  uint8_t op = vm_getbits(command, 54, 3);

  if(op) {
    fprintf(MSG_OUT, "if (");
    print_g_reg(log, vm_getbits(command,39,8));
    print_cmp_op(log, op);
    print_reg_or_data(log, command, vm_getbits(command, 55,1), 31);
    fprintf(MSG_OUT, ") ");
  }
}

static void print_if_version_2(const trace_log_t* log, command_t* command) {
  uint8_t op = vm_getbits(command, 54, 3);

  if(op) {
    fprintf(MSG_OUT, "if (");
    print_reg(log, vm_getbits(command, 15, 8));
    print_cmp_op(log, op);
    print_reg(log, vm_getbits(command, 7, 8));
    fprintf(MSG_OUT, ") ");
  }
}

static void print_if_version_3(const trace_log_t* log, command_t* command) {
  uint8_t op = vm_getbits(command, 54, 3);

  if(op) {
    fprintf(MSG_OUT, "if (");
    print_g_reg(log, vm_getbits(command, 43, 4));
    print_cmp_op(log, op);
    print_reg_or_data(log, command, vm_getbits(command, 55, 1), 15);
    fprintf(MSG_OUT, ") ");
  }
}

static void print_if_version_4(const trace_log_t* log, command_t* command) {
  uint8_t op = vm_getbits(command, 54, 3);

  if(op) {
    fprintf(MSG_OUT, "if (");
    print_g_reg(log, vm_getbits(command, 51, 4));
    print_cmp_op(log, op);
    print_reg_or_data(log, command, vm_getbits(command, 55, 1), 31);
    fprintf(MSG_OUT, ") ");
  }
...
static void print_if_version_5(const trace_log_t* log, command_t* command) {
  uint8_t op = vm_getbits(command, 54, 3);
  ...
  if(op) {
    if (set_immediate) {
      fprintf(MSG_OUT, "if (");
      print_g_reg(log, vm_getbits(command, 31, 8));
      print_cmp_op(log, op);
      print_reg(log, vm_getbits(command, 23, 8));
      fprintf(MSG_OUT, ") ");
    } else {
      fprintf(MSG_OUT, "if (");
      print_g_reg(log, vm_getbits(command, 39, 8));
...
static void print_special_instruction(const trace_log_t* log, command_t* command) {
...
static void print_linksub_instruction(const trace_log_t* log, command_t* command) {
...
static void print_link_instruction(const trace_log_t* log, command_t* command, int optional) {
  uint8_t op = vm_getbits(command, 51, 4);
  ...
  switch(op) {
    case 0:
      ...
      break;
    case 1:
      print_linksub_instruction(log, command);
      break;
    case 4:
...
static void print_jump_instruction(const trace_log_t* log, command_t* command) {
...
static void print_system_set(const trace_log_t* log, command_t* command) {
  int i;
...
/*        Surely there must be some system set command for that ? */
  ...
  switch(vm_getbits(command, 59, 4)) {
    case 1: /*  Set system reg 1 &| 2 &| 3 (Audio, Subp. Angle) */
      for(i = 1; i <= 3; i++) {
        if(vm_getbits(command, 47 - (i*8), 1)) {
          print_system_reg(log, i);
          fprintf(MSG_OUT, " = ");
          print_reg_or_data_2(log, command, vm_getbits(command, 60, 1), 47 - (i*8) );
          fprintf(MSG_OUT, " ");
        }
      }
      break;
    case 2: /*  Set system reg 9 & 10 (Navigation timer, Title PGC number) */
      print_system_reg(log, 9);
      fprintf(MSG_OUT, " = ");
      print_reg_or_data(log, command, vm_getbits(command, 60, 1), 47);
      fprintf(MSG_OUT, " ");
      print_system_reg(log, 10);
    ...
    case 3: /*  Mode: Counter / Register + Set */
      ...
      print_g_reg(log, vm_getbits(command, 19, 4));
      print_set_op(log, 0x1); /*  '=' */
      print_reg_or_data(log, command, vm_getbits(command, 60, 1), 47);
      break;
    case 6: /*  Set system reg 8 (Highlighted button) */
      print_system_reg(log, 8);
...
static void print_set_version_1(const trace_log_t* log, command_t* command) {
  uint8_t set_op = vm_getbits(command, 59, 4);

  if(set_op) {
    print_g_reg(log, vm_getbits(command, 35, 4));
    print_set_op(log, set_op);
    print_reg_or_data(log, command, vm_getbits(command, 60, 1), 31);
  } else {
    fprintf(MSG_OUT, "NOP");
  }
}

static void print_set_version_2(const trace_log_t* log, command_t* command) {
  uint8_t set_op = vm_getbits(command, 59, 4);
...
static void print_set_version_3(const trace_log_t* log, command_t* command) {
  uint8_t set_op = vm_getbits(command, 59, 4);

  if(set_op) {
    print_g_reg(log, vm_getbits(command, 51, 4));
    print_set_op(log, set_op);
    print_reg_or_data_3(log, command, vm_getbits(command, 60, 1), 47);
  } else {
    fprintf(MSG_OUT, "NOP");
  }
}
...
void vm_print_mnemonic(const trace_log_t* log, vm_cmd_t *vm_command)  {
  command_t command;
  ...
  switch(vm_getbits(&command,63,3)) { /* three first bits */
    case 0: /*  Special instructions */
      print_if_version_1(log, &command);
      print_special_instruction(log, &command);
      break;
    case 1: /*  Jump/Call or Link instructions */
      if(vm_getbits(&command,60,1)) {
        print_if_version_2(log, &command);
        print_jump_instruction(log, &command);
      } else {
        print_if_version_1(log, &command);
        print_link_instruction(log, &command, 0); /*  must be pressent */
      }
      break;
    case 2: /*  Set System Parameters instructions */
      print_if_version_2(log, &command);
      print_system_set(log, &command);
      print_link_instruction(log, &command, 1); /*  either 'if' or 'link' */
      break;
    case 3: /*  Set General Parameters instructions */
      print_if_version_3(log, &command);
      print_set_version_1(log, &command);
      print_link_instruction(log, &command, 1); /*  either 'if' or 'link' */
      break;
    case 4: /*  Set, Compare -> LinkSub instructions */
      print_set_version_2(log, &command);
      fprintf(MSG_OUT, ", ");
      print_if_version_4(log, &command);
      print_linksub_instruction(log, &command);
      break;
    case 5: /*  Compare -> (Set and LinkSub) instructions */
      print_if_version_5(log, &command);
      fprintf(MSG_OUT, "{ ");
      print_set_version_3(log, &command);
      fprintf(MSG_OUT, ", ");
      print_linksub_instruction(log, &command);
      fprintf(MSG_OUT, " }");
      break;
...
void vm_print_cmd(const trace_log_t* log, int row, vm_cmd_t *vm_command) {
  int i;
  ...
  vm_print_mnemonic(log, vm_command);
  fprintf(MSG_OUT, "\n");
}
