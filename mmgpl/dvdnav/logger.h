...
void dvdnav_critical_failure(void);
int dvdnav_assert_worker(void *priv, const dvdnav_logger_cb *logcb, const char* expression);

#define vassert(_Expression_) (void)((!!(_Expression_)) || (dvdnav_assert_worker(vm->priv, &vm->logcb, #_Expression_)))
#define dassert(_Expression_) (void)((!!(_Expression_)) || (dvdnav_assert_worker(NULL, NULL, #_Expression_)))
